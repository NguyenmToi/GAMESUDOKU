...
class LichSuaChoiState extends State<LichSuaChoi> {
  ...
  Future<void> loadData() async {
    try {
      List<cMucDoChoiNgauNhien> man = await cMucDoChoiNgauNhien.taiDanhSachMan();
      setState(() {
        sudokuList = man;
        isLoading = false;
      });
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      SizedBox(width: 5),
                                      Text('Thời gian: ${dinhDang(sudoku.thoigian)}',
                                      style: TextStyle(fontSize: 17, fontWeight: FontWeight.normal),),
                                    ],
                                  ),
                                  SizedBox(height: 20),
                                  Row(
                                    children: [
                                      Icon(Icons.error_outline_outlined, size: 25),
                                      SizedBox(width: 5),
                                      Text('Lượt sai: ${sudoku.soloi}',
                                      style: TextStyle(fontSize: 17, fontWeight: FontWeight.normal),),
                                    ],
                                  ),
                                   SizedBox(height: 20),
...
class BangSudoku extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    double kichThuocBang = MediaQuery.of(context).size.width * 0.5; // Lấy 55% chiều rộng màn hình
    return Container(
      width: kichThuocBang,
