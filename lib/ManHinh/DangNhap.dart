...
class DangNhapState extends State<DangNhap> {
  ...
  Future<void> _dangNhap() async {
    String taiKhoan = _TaiKhoan.text.trim(); // Lấy giá trị tài khoản từ TextField và xóa khoảng trắng ở đầu và cuối chuỗi
    String matKhau = _MatKhau.text.trim(); // Lấy giá trị mật khẩu từ TextField và xóa khoảng trắng ở đầu và cuối chuỗi

    // Kiểm tra nếu tài khoản hoặc mật khẩu rỗng
    if (taiKhoan.isEmpty || matKhau.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Bạn chưa nhập Tài khoản hoặc Mật khẩu"), // Hiển thị thông báo
        ),
      );
