...
class _QuanLyTaiKhoanState extends State<QuanLyTaiKhoan> {
  ...
  void CapNhatTimKiem(String query) {
    setState(() {
      searchQuery = query;
      filteredAccounts = accounts
          .where((account) => account.tentaikhoan.toLowerCase().contains(searchQuery.toLowerCase()))
          .toList();
    });
  }
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              CircleAvatar(
                                radius: 25,
                                backgroundImage: FileImage(File(filteredAccounts[index].anh)),
                              ...
                              IconButton(
                                icon: Icon(
                                  filteredAccounts[index].trangthai ? Icons.lock_open : Icons.lock,
                                  color: Colors.black,
                                ),
                                onPressed: () => ThongBaoXacNhan(filteredAccounts[index]),
                              ),
                            ],
                          ),
                        ),
