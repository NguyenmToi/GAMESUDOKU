...
class Manhinhchoimucdo extends StatefulWidget {
  ...
  const Manhinhchoimucdo({Key? key, required this.taikhoan ,required this.mucdo, required this.soan,required this.diem}) : super(key: key);
...
class _ManhinhchoimucdoState extends State<Manhinhchoimucdo> {
  ...
  List<List<bool>> oKhoiTao = List.generate(9, (_) => List.generate(9, (_) => false));
  List<List<int>> mangTam = List.generate(9, (_) => List.generate(9, (_) => 0)); // Khai báo mảng tạm để lưu trữ các giá trị
...
void xuLyOChon(int hang, int cot) {
  setState(() {
    if (oKhoiTao[hang][cot]) { // Chỉ cho phép chọn vào ô khởi tạo
      hangDuocChon = hang;
      cotDuocChon = cot;
    } else {
      hangDuocChon = null;
...
void capNhatDiemVaThemManChoi() async {
  try {
    ...
    cMucDoChoiNgauNhien.themManChoiNgauNhien(
      widget.taikhoan,
      widget.mucdo,
      5 - loiSai,
      DateTime.now(),
              ...
              Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      for (int i = 1; i <= 5; i++) xayDungSo(i), // Tạo nút số từ 1 đến 5
                    ],
                  ),
                  const SizedBox(height: 10),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      for (int i = 6; i <= 9; i++) xayDungSo(i), // Tạo nút số từ 6 đến 9
                      Ink(
                        height: 45,
                        width: 60,
                        decoration: BoxDecoration(
...
void thongBaoThoat() {
 ...
 void xoaONgauNhien() {
  ...
  for (int i = 0; i < 81; i++) {
    if (bangSudoku[i ~/ 9][i % 9] != 0) { // Kiểm tra ô đã được điền số chưa
      viTriDaDien.add(i);
    }
  }
  viTriDaDien.shuffle();
  ...
  for (int i = 0; i < widget.soan; i++) { // Số ô cần xóa
    int index = viTriDaDien[i];
    int hang = index ~/ 9;
    int cot = index % 9;
    if (!oKhoiTao[hang][cot]) { // Nếu ô không phải là ô khởi tạo
